...
fn thread_worker(name: &str, receiver: Receiver<&str>) {
    for package in receiver {
        println!("worker {} handles package #{}.", name, package);
        sleep_ms(250);
    }
}
...
fn main() {
    ...
    let bob = thread::spawn(|| thread_worker("Bob", rx1));
    let lea = thread::spawn(|| thread_worker("Lea", rx2));

    for package in vec!["1", "2", "3", "4"] {
        println!("ORDER: {}", package);
        let _ = tx1.send(package);
    }
    drop(tx1);
